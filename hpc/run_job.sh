...
# Example: CUDA_VERSION=12.9 sbatch hpc/run_job.sh ...
...
# Get project root directory (parent of hpc directory)
...
if [ ! -f "$APPTAINER_IMAGE" ]; then
    ...
    echo ""
    echo "Please ensure:"
    echo "  1. You've run ./hpc/setup_hpc.sh to build the container"
    echo "  2. The container image exists at: $PROJ_ROOT/inflamm-debate-fm.sif"
    ...
    echo "  export APPTAINER_IMAGE=/path/to/inflamm-debate-fm.sif"
    echo "  sbatch hpc/run_job.sh ..."
    exit 1
fi

# Check if run_apptainer.sh exists
if [ ! -f "$PROJ_ROOT/run_apptainer.sh" ]; then
    echo "Error: run_apptainer.sh not found. Please run ./hpc/setup_hpc.sh first"
    exit 1
fi
...
if [ "$USE_GPU" = "1" ]; then
    ...
    if command -v nvidia-smi &> /dev/null; then
        ...
        if command -v nvcc &> /dev/null; then
            echo "Host CUDA version (nvcc): $(nvcc --version | grep release | sed 's/.*release \([0-9.]*\).*/\1/')"
        else
            echo "nvcc not found on host (CUDA is in container)"
        fi
        # Check CUDA version in container if we can
        echo "Note: Container has PyTorch with CUDA 12.9 support"
    fi
else
    echo "GPU: Disabled"
fi
